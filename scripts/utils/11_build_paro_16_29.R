...
chk_cov <- cov_quarter |>
  ...
  summarise(n_trimestres = sum(n > 0), .groups="drop")
...
pesos <- epa2 |>
  group_by(ano, edad_bin) |>
  summarise(activos = mean(valor_num, na.rm = TRUE), .groups = "drop") |>
...
# Chequeo rápido de cabeceras esperadas (flexible)
req_cols <- c("edad","sexo","periodo","total")
if (!all(req_cols %in% names(paro_raw))) {
  stop("Paro: faltan columnas. Esperadas: ", paste(req_cols, collapse=", "),
       " | Detectadas: ", paste(names(paro_raw), collapse=", "))
}

# Filtra Ambos sexos y mapea bandas EXACTAS según fichero
...
readr::write_csv(
  paro_raw |> distinct(edad, sexo, periodo) |> head(200),
...
if (!all(c("16-19","20-24","25-29") %in% bands_present)) {
  vals_edad <- paste(utils::head(sort(unique(paro_raw$edad)), 50), collapse = " | ")
  stop(paste0(
    "Paro: el fichero no trae bandas 16–19/20–24/25–29.\n",
    "Valores detectados en 'edad': ", vals_edad, "\n",
...
df2 <- df_paro |>
  filter(!is.na(edad_band), !is.na(tasa), !is.na(ano), !is.na(trimestre), ano %in% ANOS) |>
...
pesos_adj <- out_activos |>
  transmute(ano, edad_band = edad_bin, peso = activos) |>
  filter(edad_band %in% c("16-19","20-24","25-29")) |>
  arrange(ano, edad_band)
...
paro_pond <- left_join(rates_year, pesos_adj, by = c("ano","edad_band")) |>
  group_by(ano) |>
  summarise(
...
paro_15_29 <- paro_pond |> filter(ano %in% ANOS) |> select(ano, paro_15_29, metodo)
