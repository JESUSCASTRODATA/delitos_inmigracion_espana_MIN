...
#
# Propósito:
#   Verificar automáticamente la integridad de los datasets en data/processed/:
#   cobertura anual, duplicados, no-negatividad, HE≤HC, det_ext≤det_tot,
#   rangos de porcentajes y cierres contables de población.
#
# Uso:
#   Rscript scripts/03_qa_global_integridad.R
#   (o desde R) source("scripts/03_qa_global_integridad.R")
#
# Salidas principales:
#   - output/tables/qa_summary_checks.csv   ← resumen PASS/WARN/FAIL
#   - output/tables/qa_cov_compilada.csv    ← cobertura por dataset
#   - tablas de violaciones/diagnóstico específicas si aplica
################################################################################
...
safe_read_csv <- function(path) if (file.exists(path)) suppressMessages(readr::read_csv(path, show_col_types = FALSE)) |> janitor::clean_names() else tibble()
write_clean <- function(df, path, na="") {
  dir.create(dirname(path), recursive = TRUE, showWarnings = FALSE)
  readr::write_csv(df, path, na = na); message("Escrito: ", normalizePath(path, winslash="/", mustWork = FALSE))
  invisible(path)
}
...
read_raw <- function(path, delim=";") {
  readr::read_delim(path, delim = delim, col_types = cols(.default = col_character()),
                    show_col_types = FALSE, trim_ws = TRUE) |> janitor::clean_names()
}
...
scan_read_problems <- function(files) {
  purrr::map_dfr(files, function(fp) {
    try1 <- try(
      readr::read_delim(fp, delim = ";", col_types = cols(.default = col_character()),
    ...
    if (inherits(try1, "try-error")) {
      return(tibble(archivo = basename(fp), sep = NA_character_, n_problems = NA_integer_))
    }
    probs <- readr::problems(try1)
    tibble(archivo = basename(fp), sep = sep_used, n_problems = nrow(probs))
  })
}
files_raw <- if (dir.exists(raw_dir)) list.files(raw_dir, "\\.csv$", full.names = TRUE) else character()
probs_df  <- if (length(files_raw)) scan_read_problems(files_raw) else tibble(archivo=character(), sep=character(), n_problems=integer())
...
dup_raw_reg <- if (nrow(map_reg)) map_reg |> count(raw_region, name="n") |> filter(n>1) else tibble()
...
variantes_ccaa <- if (nrow(map_reg)) map_reg |> count(propuesta_region, name="n_variantes") else tibble(propuesta_region=character(), n_variantes=integer())
...
# 1a) Fuera de catálogo REAL (ni CCAA ni whitelist, y no NA)
fuera_final <- if (nrow(map_reg)) map_reg |> filter(!is.na(propuesta_region) & !(propuesta_region %in% regiones_permitidas)) else tibble()
...
# 1b) NAs en propuesta_region (deben resolverse)
na_en_propuesta <- if (nrow(map_reg)) map_reg |> filter(is.na(propuesta_region) | propuesta_region=="") else tibble()
...
# (INFO) Diagnóstico previo del paso 02: regiones fuera de catálogo detectadas en RAW
...
dup_tip <- if (nrow(map_tip)) map_tip |> count(propuesta_tipo, name="n") |> filter(n>1) else tibble()
out_dup_tip <- here_path("output","tables","qa_map_tipologias_duplicados.csv")
write_clean(dup_tip, out_dup_tip)
summary_rows[[length(summary_rows)+1]] <- add_row_df(
...
revalida_anos <- function(archivo) {
  ...
  dat <- suppressMessages(readr::read_csv(fp, show_col_types = FALSE)) |> janitor::clean_names()
...
if (nrow(resumen_raw)) {
  off_years_raw <- resumen_raw |> filter(!is.na(anos_fuera_2010_2023) & anos_fuera_2010_2023 > 0)
  if (nrow(off_years_raw)) {
    reval <- tibble(archivo = off_years_raw$archivo,
                    fuera_en_processed = vapply(off_years_raw$archivo, revalida_anos, integer(1)))
    still_bad <- reval |> filter(!is.na(fuera_en_processed) & fuera_en_processed > 0)
    out_off_years <- here_path("output","tables","qa_raw_anos_fuera_2010_2023.csv")
    write_clean(off_years_raw |> left_join(reval, by = "archivo"), out_off_years)
    summary_rows[[length(summary_rows)+1]] <- add_row_df(
...
if (nrow(parse_issues)) {
  ...
  has_digits <- if (!is.na(tok_col)) any(grepl("\\d", parse_issues[[tok_col]])) else FALSE
...
hc   <- if (file.exists(here_path("data","raw","hechos_conocidos.csv"))) read_raw(here_path("data","raw","hechos_conocidos.csv")) else tibble()
he   <- if (file.exists(here_path("data","raw","hechos_esclarecidos.csv"))) read_raw(here_path("data","raw","hechos_esclarecidos.csv")) else tibble()
...
if (nrow(hc) && nrow(he) && nrow(map_reg) && nrow(map_tip)) {
  ...
  map_reg_slim <- map_reg |> select(raw_region, propuesta_region)
  map_tip_slim <- map_tip |> select(raw_tipo,   propuesta_tipo)
  
  hc2 <- hc |>
    mutate(ano = extract_year(.data[[col_ano_hc]]),
           tipo = .data[[col_tipo_hc]], region_raw = .data[[col_reg_hc]],
           valor = sfnum(.data[[col_val_hc]])) |>
    left_join(map_tip_slim, by = c("tipo" = "raw_tipo")) |>
    left_join(map_reg_slim, by = c("region_raw" = "raw_region")) |>
    transmute(ano, region = propuesta_region, tipo = propuesta_tipo, conocidos = valor) |>
    filter(!is.na(ano), !is.na(region), !is.na(tipo))
  
  he2 <- he |>
    mutate(ano = extract_year(.data[[col_ano_he]]),
           tipo = .data[[col_tipo_he]], region_raw = .data[[col_reg_he]],
           valor = sfnum(.data[[col_val_he]])) |>
    left_join(map_tip_slim, by = c("tipo" = "raw_tipo")) |>
    left_join(map_reg_slim, by = c("region_raw" = "raw_region")) |>
    transmute(ano, region = propuesta_region, tipo = propuesta_tipo, esclarecidos = valor) |>
    filter(!is.na(ano), !is.na(region), !is.na(tipo))
  
  chk_he_hc <- hc2 |>
    full_join(he2, by = c("ano","region","tipo")) |>
    tidyr::replace_na(list(conocidos=0, esclarecidos=0)) |>
    mutate(flag = esclarecidos <= conocidos, delta = esclarecidos - conocidos) |>
    arrange(ano, region, tipo)
  
  viol_he_hc <- chk_he_hc |> filter(!flag)
...
det_tot <- if (file.exists(here_path("data","raw","detenciones_tipologia.csv"))) read_raw(here_path("data","raw","detenciones_tipologia.csv")) else tibble()
det_ext <- if (file.exists(here_path("data","raw","detenciones_extranjeros_tipologia.csv"))) read_raw(here_path("data","raw","detenciones_extranjeros_tipologia.csv")) else tibble()
...
if (nrow(det_tot) && nrow(det_ext) && nrow(map_tip)) {
  ...
  dt2 <- det_tot |>
    transmute(ano = extract_year(.data[[col_ano_dt]]), tipo = .data[[col_tipo_dt]], total = sfnum(.data[[col_val_dt]])) |>
    left_join(map_tip |> select(raw_tipo, propuesta_tipo), by = c("tipo"="raw_tipo")) |>
    transmute(ano, tipo = propuesta_tipo, total) |>
    filter(!is.na(ano), !is.na(tipo))
  
  de2 <- det_ext |>
    transmute(ano = extract_year(.data[[col_ano_de]]), tipo = .data[[col_tipo_de]], ext = sfnum(.data[[col_val_de]])) |>
    left_join(map_tip |> select(raw_tipo, propuesta_tipo), by = c("tipo"="raw_tipo")) |>
    transmute(ano, tipo = propuesta_tipo, ext) |>
    filter(!is.na(ano), !is.na(tipo))
  
  chk_det <- dt2 |>
    group_by(ano, tipo) |> summarise(total = sum(total, na.rm=TRUE), .groups="drop") |>
    inner_join(de2 |> group_by(ano, tipo) |> summarise(ext = sum(ext, na.rm=TRUE), .groups="drop"), by=c("ano","tipo")) |>
    mutate(flag = ext <= total, delta = ext - total) |>
    arrange(ano, tipo)
  
  viol_det <- chk_det |> filter(!flag)
...
nn_chk <- bind_rows(
  if (exists("hc2")) hc2 |> transmute(dataset="hechos_conocidos", ano, region, tipo, valor = conocidos) else tibble(),
  if (exists("he2")) he2 |> transmute(dataset="hechos_esclarecidos", ano, region, tipo, valor = esclarecidos) else tibble(),
  if (exists("dt2")) dt2 |> transmute(dataset="detenciones_total", ano, region=NA_character_, tipo, valor = total) else tibble(),
  if (exists("de2")) de2 |> transmute(dataset="detenciones_extranjeros", ano, region=NA_character_, tipo, valor = ext) else tibble()
)

if (nrow(nn_chk)) {
  neg <- nn_chk |> filter(!is.na(valor) & valor < 0)
  out_neg <- here_path("output","tables","qa_valores_negativos.csv")
  if (nrow(neg)) write_clean(neg, out_neg)
  na_keys <- nn_chk |> filter(is.na(ano) | is.na(tipo) | (dataset %in% c("hechos_conocidos","hechos_esclarecidos") & is.na(region)))
...
if (file.exists(parse_path)) {
  probs <- safe_read_csv(parse_path) |> rename(token = token_muestra)
  write_clean(probs, here_path("output","tables","qa_parse_tokens_muestras.csv"))
  top_tokens <- probs |> count(token, sort = TRUE)
  by_filecol <- probs |> count(archivo, columna, sort = TRUE)
  examples3  <- probs |> group_by(token) |> slice_head(n = 3) |> ungroup() |> arrange(token)
