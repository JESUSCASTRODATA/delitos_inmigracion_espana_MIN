...
suppressPackageStartupMessages({
  ...
  library(rlang); library(scales)  # Para eje X en %
  # (opcional) ggrepel para etiquetas más limpias si está instalado
})
...
# ===== Alias Δ y creación de Δ si faltan =====
...
run_block <- function(df, label) {
  ...
  # --- Scatter con eje % + anotaciones + subtítulo r/p/n + residual plot ----
  plot_pairs <- function(df, xs, ys, prefix) {
    ...
    for (xv in xs) for (yv in ys) {
      ...
      subt <- ""
      if (nrow(d2) >= 3) {
        ct <- suppressWarnings(cor.test(d2$x, d2$y, method = "pearson", exact = FALSE))
        subt <- sprintf("Pearson r=%.2f, p=%.3g, n=%d", unname(ct$estimate), unname(ct$p.value), nrow(d2))
      }
      
      # Eje % si la X es proporción (p.ej., pct_extranjeros 0–1)
      fmt_percent <- function(p) scales::percent(p, accuracy = 0.1)  # 1 decimal
      is_prop <- is.numeric(d2$x) && max(d2$x, na.rm=TRUE) <= 1.5 && min(d2$x, na.rm=TRUE) >= 0
      
      # Etiquetas para años COVID
      lab_covid <- dplyr::filter(d2, ano %in% c(2020, 2021))
      
      # Scatter principal
      base_scatter <- ggplot(d2, aes(x, y)) +
        geom_point() +
        { if (nrow(d2) >= 3) geom_smooth(method = "lm", se = TRUE) else NULL } +  # banda de confianza
        labs(
          title = sprintf("%s vs %s (%s)", yv, xv, label),
          subtitle = subt,
          x = xv, y = yv,
        ...
        (if (is_prop) scale_x_continuous(labels = fmt_percent) else NULL)
      
      p <- base_scatter +
        {
          if (nrow(lab_covid)) {
            if (have_ggrepel) ggrepel::geom_text_repel(data = lab_covid, aes(label = ano), size = 3, max.overlaps = 20)
            else geom_text(data = lab_covid, aes(label = ano), size = 3, vjust = -0.6)
          } else NULL
        }
      ...
      if (nrow(d2) >= 3) {
        ...
        lab_out <- dplyr::filter(aug, abs(resid_std) > 2 | ano %in% c(2020, 2021))
        ...
        p_res <- ggplot(aug, aes(fitted, resid)) +
          ...
          {
            if (nrow(lab_out)) {
              if (have_ggrepel) ggrepel::geom_text_repel(data = lab_out, aes(label = ano), size = 3, max.overlaps = 20)
              else geom_text(data = lab_out, aes(label = ano), size = 3, vjust = -0.6)
            } else NULL
          } +
          labs(
            title = sprintf("Residuos: %s ~ %s (%s)", yv, xv, label),
            x = "Ajustado", y = "Residuo"
          ) +
          (if (exists("theme_pearl_lightgrid")) theme_pearl_lightgrid() else theme_minimal(base_size = 11))
        
        out_res <- file.path(out_fig, sprintf("20_%s_%s_vs_%s_%s_residuals.png", prefix, yv, xv, label))
        suppressMessages(
          (if (exists("ggsave_pearl")) ggsave_pearl else ggsave)(
            filename = out_res, plot = p_res, width = 6, height = 4, dpi = 120
          )
        )
      }
    }
...
mini_check_20_pb <- function(
    out_tab = here::here("output","tables"),
    out_fig = here::here("output","figures")
){
  suppressPackageStartupMessages({ library(readr); library(janitor); library(fs); library(here); library(tibble); library(dplyr) })
  cat("\n—— MINI CHECK · 20_preguntas_basicas ————————————————\n")
  
  # 1) Salidas esperadas
  exp_csv <- c(
    "20_corr_niveles_all.csv",
    "20_corr_deltas_all.csv",
    "20_corr_niveles_sin_covid.csv",
    "20_corr_deltas_sin_covid.csv"
  )
  paths  <- file.path(out_tab, exp_csv)
  existe <- file.exists(paths)
  n_rows <- vapply(paths, function(p){
  ...
  exists_df <- tibble(archivo = exp_csv, path = paths, existe = existe, n_rows = n_rows)
  print(exists_df, n = nrow(exists_df))
  
  # 2) Preview rápido (niveles · all)
  pth <- file.path(out_tab, "20_corr_niveles_all.csv")
  if (file.exists(pth)) {
    d <- suppressMessages(readr::read_csv(pth, show_col_types = FALSE)) |> janitor::clean_names()
    need <- c("x","y","pearson_r","pearson_p","n")
    if (all(need %in% names(d))) {
      cat("\nTop preview (niveles · all):\n")
      prev <- d |>
        transmute(x, y,
                  r = round(as.numeric(pearson_r), 3),
                  p = signif(as.numeric(pearson_p), 3),
                  n = as.integer(n)) |>
        arrange(dplyr::desc(abs(r)))
      print(prev, n = nrow(prev))
    }
  }
  
  # Recuento de figuras (recursivo y con regex correcto)
  n_png <- if (fs::dir_exists(out_fig)) {
    length(fs::dir_ls(out_fig, recurse = TRUE, type = "file",
                      regexp = "20_(niv|del)_.+\\.png$"))
  } else 0L
  cat("\nFiguras 20_* detectadas: ", n_png, "\n", sep = "")
  
  # (Opcional) muestra 3 ejemplos
  if (n_png > 0) {
    ej <- basename(fs::dir_ls(out_fig, recurse = TRUE, type = "file",
                              regexp = "20_(niv|del)_.+\\.png$")[1:min(3, n_png)])
    cat("Ejemplos: ", paste(ej, collapse = " · "), "\n", sep = "")
  }
  
  
  
  # 4) Resumen MD
  md <- file.path(out_tab, "20_pb_summary.md")
  cat("Resumen MD: ", if (file.exists(md)) "OK" else "NO ENCONTRADO", "\n", sep = "")
  if (file.exists(md)) {
    cat("— Primeras 10 líneas —\n")
    cat(paste(utils::head(readLines(md, warn = FALSE), 10), collapse = "\n"), "\n")
  }
