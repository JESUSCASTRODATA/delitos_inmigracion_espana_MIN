...
## ==== Listado de ficheros (único y ordenado) ================================
...
for (f in files) {
  ...
  # Intento de lectura con captura fina de warnings
  df <- NULL
  withCallingHandlers({
  ...
  # Si falló la lectura, registramos y pasamos al siguiente archivo
  if (read_error) {
    elap <- (proc.time() - t0)[["elapsed"]]
    resumen[[length(resumen) + 1]] <- tibble::tibble(
      archivo = basename(f),
      filas = NA_integer_, cols = NA_integer_,
      bytes = tryCatch(file.size(f), error = function(e) NA_real_),
      mb = human_mb(tryCatch(file.size(f), error = function(e) NA_real_)),
      sep = NA_character_, encoding = NA_character_,
      col_periodo = NA_character_,
      ano_min = NA_integer_, ano_max = NA_integer_,
      n_valores_ano = NA_integer_,
      anos_fuera_2010_2023 = NA_integer_
    )
    file_logs[[length(file_logs) + 1]] <- tibble::tibble(
      archivo = basename(f),
  ...
  # A partir de aquí, 'df' es un data.frame válido
  ...
  # Esquema de columnas
  if (WRITE_DERIVADOS) {
    write_clean(tibble::tibble(columna = nms),
  ...
  # Parseo numérico en candidatas (más estricto y sin placeholders)
  ...
  for (ct in cand_totals) {
    ...
    vals <- as.character(tot_raw[non_empty])
    
    # 1) ¿Parecen números? (evita columnas categóricas tipo “componentes”)
    looks_numeric <- grepl("(?<!\\d)\\d{1,3}(?:[\\.,]\\d{3})*(?:[\\.,]\\d+)?%?(?!\\d)", vals, perl = TRUE)
    if (length(looks_numeric) && mean(looks_numeric) < 0.25) next  # ← skip columna no numérica
    
    parsed <- parse_total_es(tot_raw)
    
    # 2) No contemos placeholders como incidencias
    raw_chr <- tolower(trimws(as.character(tot_raw)))
    is_placeholder <- raw_chr %in% .NUM_MISS
    
    problem_mask <- is.na(parsed) & non_empty & !is_placeholder
    na_new <- sum(problem_mask)
    
    if (na_new > 0) {
      bad_tokens <- unique(na.omit(as.character(tot_raw[problem_mask])))
      parse_iss[[length(parse_iss) + 1]] <- tibble::tibble(
        archivo = basename(f),
        columna = ct,
        n_na_nuevos = na_new,
    ...
    # Heurística: si menos del 20% de los no vacíos tienen dígitos, tratamos la columna como textual
    v_chr   <- as.character(vec)
    v_chr_s <- trimws(v_chr)
    non_na  <- !is.na(v_chr_s) & nzchar(v_chr_s)
    frac_digits <- if (sum(non_na)) mean(grepl("\\d", v_chr_s[non_na])) else 0
    if (!is.finite(frac_digits) || frac_digits < 0.20) next
    
    parsed   <- parse_total_es(vec)
    raw_chr  <- tolower(v_chr_s)
    non_empty <- non_na & !(raw_chr %in% MISS_TOK)
    
    na_new <- sum(is.na(parsed) & non_empty)
    if (na_new > 0) {
      bad_tokens <- unique(na.omit(as.character(vec[is.na(parsed) & non_empty])))
      parse_iss[[length(parse_iss) + 1]] <- tibble::tibble(
        archivo = basename(f),
        columna = ct,
        n_na_nuevos = na_new,
  ...
  uniques_df <- tibble::tibble(columna = character(), n_unicos = integer(), muestra = character())
  for (nm in nms) {
    vec <- df[[nm]]
    if (key_col_pattern(nm) || is_categorical_like(vec, nrow(df))) {
      uniques_df <- dplyr::bind_rows(
        uniques_df,
        tibble::tibble(columna = nm,
  ...
  # Log de tiempos/warnings de este archivo
  elap <- (proc.time() - t0)[["elapsed"]]
  file_logs[[length(file_logs) + 1]] <- tibble::tibble(
    archivo = basename(f),
    segundos = round(elap, 3),
...
acciones_df <- if (length(acciones)) dplyr::bind_rows(acciones) %>% dplyr::arrange(archivo, issue) else tibble::tibble()
