...
suppressPackageStartupMessages({
  ...
  library(here)
  library(fs)
  library(purrr)   # <- NECESARIO: usado en find_numeric_parse_issues()
})
...
`%||%` <- function(a, b) if (is.null(a) || (is.atomic(a) && all(is.na(a)))) b else a
...
safe_parse_number <- function(x) {
  ...
  x_clean <- gsub("(\u00A0|\u0020)", "", x_chr)
  x_clean <- gsub("(?<=\\d)[,](?=\\d{3}(\\D|$))", "", x_clean, perl = TRUE)
  x_clean <- gsub("(?<=\\d)[.](?=\\d{3}(\\D|$))", "", x_clean, perl = TRUE)
  dual <- grepl(",", x_clean) & grepl("\\.", x_clean)
  x_clean[dual] <- sub(",", "", x_clean[dual])
...
find_numeric_parse_issues <- function(df, archivo = NA_character_) {
  ...
  purrr::map_dfr(cols, function(col) {
    ...
    parsed <- limpia_num(v)
    has_digits <- grepl("\\d.*\\d", v) # al menos 2 dígitos para evitar falsos
    ...
    tibble::tibble(
      ...
      token_muestra  = utils::head(toks, 1)
    )
  })
}
...
join_with_log <- function(x, y, by = "ano", mode = c("left", "inner")) {
  ...
  # Solo mostramos la primera clave del vector 'by' para mensajes legibles
  by_show <- if (length(by) >= 1) by[1] else "(clave)"
  
  if (nrow(miss_in_y))
    message("[JOIN] claves de X sin match en Y por ", by_show, ": ",
            paste(utils::head(miss_in_y[[by_show]], 5), collapse = ", "),
            if (nrow(miss_in_y) > 5) " ...")
  
  if (nrow(miss_in_x))
    message("[JOIN] claves de Y sin match en X por ", by_show, ": ",
            paste(utils::head(miss_in_x[[by_show]], 5), collapse = ", "),
            if (nrow(miss_in_x) > 5) " ...")
...
load_series <- function(fp, out_name, candidates, year_col_candidates = NULL) {
  ...
  if (is.na(col_year)) col_year <- intersect(year_col_candidates, names(df))[1]
