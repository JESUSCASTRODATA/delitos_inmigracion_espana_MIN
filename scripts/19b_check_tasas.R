...
# -*- coding: UTF-8 -*-
###############################################################################
# 19b_check_tasas.R — QC + Preflight para ARDL/VAR del master de tasas
#
...
# 1) Prepara vectores fuera del data mask de dplyr
has_share_col <- "share_det_ext_sobre_tot" %in% names(df)
has_counts    <- all(c("det_ext","det_tot") %in% names(df))
has_ratio_col <- "ratio_tasa_ext_vs_tot" %in% names(df)
has_rates     <- all(c("det_ext_rate","det_tot_rate") %in% names(df))

share_vec <- if (has_share_col) {
  df$share_det_ext_sobre_tot
} else if (has_counts) {
  df$det_ext / pmax(df$det_tot, 1)         # solo si tienes conteos
} else {
  rep(NA_real_, nrow(df))                   # no hay conteos -> no podemos calcular el share
}

ratio_vec <- if (has_ratio_col) {
  df$ratio_tasa_ext_vs_tot
} else if (has_rates) {
  df$det_ext_rate / pmax(df$det_tot_rate, 1e-9)  # relación de tasas (proxy), siempre disponible
} else {
...
det_share <- df %>%
  transmute(
    ano,
    share_det_ext_sobre_tot = share_vec,   # <- ya calculado fuera
    det_ext_rate,
    det_tot_rate,
    ratio_tasa_ext_vs_tot = ratio_vec      # <- ya calculado fuera
  )

# 2) Resúmenes por ventanas
sum_det <- bind_rows(
  det_share %>% summarise(
    window = "2010_2023",
    n = n(),
  ...
  det_share %>% filter(!(ano %in% c(2020, 2021))) %>%
    summarise(
      window = "sin_covid",
      n = n(),
      mean_share_det_ext    = mean(share_det_ext_sobre_tot, na.rm = TRUE),
