...
# ---- QC rápido para core_indicadores_con_pib.csv (solo consola) -------------
suppressPackageStartupMessages({
  library(readr); library(dplyr); library(janitor); library(glue)
})

qc18merge_console <- function(
    fp = "data/processed/core_indicadores_con_pib.csv",
    year_min = 2010L, year_max = 2023L,
    yoy_warn = 0.12  # 12% umbral de aviso
){
  if (!file.exists(fp)) { cat("❌ Falta", fp, "\n"); return(invisible(FALSE)) }
  df <- readr::read_csv(fp, show_col_types = FALSE) |> janitor::clean_names()
  
  # Detecta la(s) columna(s) de índice generadas (p.ej. pib_pc_real_idx2010)
  idx_cols <- names(df)[grepl("^pib_pc_real_idx", names(df))]
  idx_col  <- if (length(idx_cols)) idx_cols[1] else NA_character_
  ...
  # Rangos y NA
  rng_pib <- range(df$pib_pc_real, na.rm = TRUE)
  rng_yoy <- range(df$pib_pc_real_yoy, na.rm = TRUE)
  na_pib  <- sum(is.na(df$pib_pc_real))
  na_yoy  <- sum(is.na(df$pib_pc_real_yoy))
  na_idx  <- if (!is.na(idx_col)) sum(is.na(df[[idx_col]])) else NA_integer_
  
  # Avisos YoY fuera de tolerancia
  yoy_bad <- df |> filter(!is.na(pib_pc_real_yoy) & abs(pib_pc_real_yoy) > yoy_warn) |>
    transmute(ano, yoy = pib_pc_real_yoy)
  
  # Imprime
  cat("\n🧪 QC18 MERGE — core + PIB\n")
  cat(glue("• Cobertura: {length(yrs)}/{length(year_min:year_max)} ({ifelse(length(yrs)>0,min(yrs),NA)}–{ifelse(length(yrs)>0,max(yrs),NA)})  faltan: {ifelse(length(miss)==0,'ninguno',paste(miss,collapse=', '))}\n"))
  cat(glue("• Rango PIB_pc_real: [{sprintf('%.2f', rng_pib[1])}, {sprintf('%.2f', rng_pib[2])}]  |  NA(PIB): {na_pib}\n"))
  cat(glue("• Rango YoY PIB: [{sprintf('%.1f', 100*rng_yoy[1])}%, {sprintf('%.1f', 100*rng_yoy[2])}%]  |  NA(YoY): {na_yoy}\n"))
  cat(glue("• Columna índice: {ifelse(is.na(idx_col),'(no encontrada)', idx_col)}  |  NA(índice): {ifelse(is.na(idx_col),'NA', na_idx)}\n"))
  
  if (nrow(yoy_bad)) {
    cat(glue("• Avisos YoY > {100*yoy_warn}%: {nrow(yoy_bad)} año(s) → ")); print(yoy_bad, n=nrow(yoy_bad))
  } else {
    cat(glue("• Avisos YoY > {100*yoy_warn}%: ninguno\n"))
  }
  
  cat("\n• Preview (últimos 5):\n")
  cols_preview <- c("ano","pib_pc_real","pib_pc_real_yoy")
  if (!is.na(idx_col)) cols_preview <- c(cols_preview, idx_col)
  print(utils::tail(df |> dplyr::select(dplyr::all_of(cols_preview)), 5), n=5)
  
  ok <- length(miss)==0 && is.finite(rng_pib[1]) && is.finite(rng_yoy[1])
  cat(if (ok) "\n✅ PASS — Merge OK a ojo\n" else "\n❌ Revisar cobertura/NA\n")
  invisible(ok)
}
