...
# Anualiza por nacionalidad (preferencia JULIO o media)
ann <- df %>%
...
out <- tot %>%
  full_join(esp, by="ano") %>%
  full_join(ext_direct, by="ano") %>%
...
# Controles (avisos)
med_tot <- median(out$poblacion_total, na.rm=TRUE)
if (is.finite(med_tot) && !(3.5e7 <= med_tot && med_tot <= 5.5e7))
  warning("Plausibilidad: mediana de total fuera de [35M, 55M]. Revisa el RAW.")

gap <- abs(out$poblacion_total - (out$poblacion_espanola + out$poblacion_extranjera))
if (any(gap > pmax(2, 0.005*out$poblacion_total), na.rm = TRUE))
  warning("Cierre: Total != Española + Extranjera (>0.5% o >2 personas) en algún año (se derivó por diferencia si faltaba).")
